...
function DibujarPetalo(x, y, RadioX, scala, Rotacion, color, pasos) {
    ...
    // Establece el color de relleno y el borde
    ctx.fillStyle = color;
    ctx.strokeStyle = 'black';
    ctx.lineWidth = 1;
    
    // Rellena el pétalo y dibuja el borde
    ctx.fill();
    ctx.stroke();
...
function DibujarFlor(x, y, NumeroPetalos, RadioXPetalo, RadioYPetalo, AltoTrazo) {
    ...
    function dibujarSiguientePetalo() {
        if (contadorPetalos <= NumeroPetalos) {
            ...
            const colores = ['red', 'lightblue', 'white', 'purple', 'pink']; // Colores posibles
            const color = colores[Math.floor(Math.random() * colores.length)]; // Color aleatorio
            DibujarPetalo(x, y, RadioXPetalo, 2, Angulo, color, 100);
            contadorPetalos++;
...
function DibujarFlorSinTallo(x, y, NumeroPetalos, RadioXPetalo, RadioYPetalo, AltoTrazo) {
    // No se dibuja el tallo en esta función
    ...
    function dibujarSiguientePetalo() {
        if (contadorPetalos < NumeroPetalos) {
            ...
            // Dibuja el centro de la flor después de todos los pétalos
            ctx.beginPath();
            ctx.arc(x, y, 10, 0, Math.PI * 2);
            ctx.fillStyle = 'yellow';
            ctx.fill();
...
function CrearVarias() {
    ...
    for (let i = 0; i < numFlores; i++) { // Ajusta el límite del bucle
